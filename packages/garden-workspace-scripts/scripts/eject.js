...
console.log(
  chalk.cyan.bold(
    'NOTE: Create Garden Ws 2+ supports TypeScript, Sass, CSS Modules and more without ejecting: ' +
...
prompts({
  ...
  if (appPackage.devDependencies) {
    // We used to put garden-workspace-scripts in devDependencies
  ...
  if (fs.existsSync(paths.appTypeDeclarations)) {
    try {
      ...
      // Remove garden-workspace-scripts reference since they're getting a copy of the types in their project
      content =
        content
          // Remove garden-workspace-scripts types
          .replace(
            /^\s*\/\/\/\s*<reference\s+types.+?"garden-workspace-scripts".*\/>.*(?:\n|$)/gm,
            ''
          )
          .trim() + os.EOL;
  ...
  if (ownPath.indexOf(appPath) === 0) {
    try {
      // remove garden-workspace-scripts and garden-workspace-scripts binaries from app node_modules
      Object.keys(ownPackage.bin).forEach(binKey => {
  ...
  if (fs.existsSync(paths.yarnLockFile)) {
    const windowsCmdFilePath = path.join(
      ...
      'node_modules',
      '.bin',
      'garden-workspace-scripts.cmd'
    );
    let windowsCmdFileContent;
    if (process.platform === 'win32') {
      // https://github.com/facebook/create-react-app/pull/3806#issuecomment-357781035
      // Yarn is diligent about cleaning up after itself, but this causes the garden-workspace-scripts.cmd file
