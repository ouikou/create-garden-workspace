...
// Finally, pack garden-workspace-scripts.
...
const scriptsFileName = cp
  .execSync(`npm pack`, { cwd: path.join(packagesDir, 'garden-workspace-scripts') })
  .toString()
  .trim();
const scriptsPath = path.join(packagesDir, 'garden-workspace-scripts', scriptsFileName);
const args = process.argv.slice(2);

// Now run the CRA command
const craScriptPath = path.join(packagesDir, 'create-garden-workspace', 'index.js');
cp.execSync(
